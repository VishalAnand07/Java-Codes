...
public class BST< T extends Comparable<T>> {
    ...
    public void range(T start,T end, Node node){
        if(node==null){
            return;
        }
    ...
    public void populatedFromSorted(T[] sorted){
        this.root=populatedFromSorted(sorted ,0,sorted.length-1);
    }

    private Node populatedFromSorted(T[] sorted, int start, int end) {
        if(start>end){
            return null;
        }
        int mid =(start+end)/2;
        Node node=new Node(sorted[mid]);
        node.left=populatedFromSorted(sorted, start, mid-1);
        node.right=populatedFromSorted(sorted,mid+1,end);
        return node;
    }
